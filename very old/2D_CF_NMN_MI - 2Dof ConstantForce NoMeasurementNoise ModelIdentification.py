...
class BayesianSINDy:
    ...
    def construct_library(self, X):
        """
        Construct the library of candidate functions (Theta matrix), including nonlinear terms.
        """
        x1 = X[:, 0]
        v1 = X[:, 1]
        x2 = X[:, 2]
        ...
        # Add quadratic terms (include all combinations)
        for i in range(len(variables)):
            for j in range(i, len(variables)):
                Theta.append(variables[i] * variables[j])
    ...
    def fit(self, X, X_dot, n_steps=5000):
        ...
        # Increase the number of walkers (at least 2 * n_params)
        self.n_walkers = max(2 * self.n_params, self.n_walkers)
    
        # Initial guess for parameters
        # Start with random small values for coefficients and 1.0 for sigma_epsilon
        initial_guess = np.concatenate([
            0.1 * np.random.randn(self.n_features),  # coeffs_f1
            0.1 * np.random.randn(self.n_features),  # coeffs_f2
            np.array([1.0])  # sigma_epsilon
        ])
    
        # Increase the spread of the initial positions
        pos = initial_guess + 0.5 * np.random.randn(self.n_walkers, self.n_params)
        ...
        sampler.reset()
        # Sampling phase without specifying initial state (continues from last)
        ...
        for theta in tqdm(theta_samples, desc="Generating predictions"):
            ...
            for i in range(len(t) - 1):
                ...
                for var in variables:
                    Theta_current.append(var)
                # Quadratic terms
                for m in range(len(variables)):
                    for n in range(m, len(variables)):
                        Theta_current.append(variables[m] * variables[n])
...
if __name__ == "__main__":
    ...
    model = BayesianSINDy(n_walkers=256, b=1)
