...
for action in range(action_size):
    q_values_for_action = Q_values[:, :, action]
    
    plt.figure(figsize=(10, 8))
    sns.heatmap(q_values_for_action.T, cmap='coolwarm', annot=False, cbar=True,
                xticklabels=np.round(displacement_values, 3),
                yticklabels=np.round(velocity_values, 3))
...
plt.figure(figsize=(10, 8))
sns.heatmap(best_actions.T, cmap='viridis', annot=False, cbar=True,
            xticklabels=np.round(displacement_values, 3),
            yticklabels=np.round(velocity_values, 3))
