...
def simulate_true(
    ...
    mu_ou1=0.0,           # OU mean for mass 1
    mu_ou2=0.0,           # OU mean for mass 2
    sigma_epsilon_1=0.1,
    sigma_epsilon_2=0.1,
    ...
    noise_array_1 = generate_ornstein_uhlenbeck_noise(
        t, theta=theta_ou, sigma=sigma_epsilon_1, mu=mu_ou1, x0=mu_ou1, seed=seed
    )
    noise_array_2 = generate_ornstein_uhlenbeck_noise(
        t, theta=theta_ou, sigma=sigma_epsilon_2, mu=mu_ou2, x0=mu_ou2, seed=seed+1 if seed else None
    )
    
    
    # (Optional) quick plots of noise for demonstration
    plt.figure(figsize=(10, 4))
    plt.plot(t, noise_array_1, 'k-', label="Noise mass1")
    plt.plot(t, noise_array_2, 'r-', label="Noise mass2")
    plt.xlabel("Time")
