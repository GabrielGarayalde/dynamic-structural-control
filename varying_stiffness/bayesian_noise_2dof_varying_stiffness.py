...
class BayesianNoiseEstimator:
    ...
    def log_prior(self, theta):
        ...
        if sigma1 <= 0 or sigma2 <= 0:
            return -np.inf
        scale = 1.0
        # -log(pi * scale * (1 + (x/scale)^2)) for each
        logp1 = -np.log(np.pi * scale * (1.0 + (sigma1/scale)**2))
        logp2 = -np.log(np.pi * scale * (1.0 + (sigma2/scale)**2))
        return logp1 + logp2

    def log_likelihood(self, theta, X, X_dot, U=None, t=None):
        """
        Compare v1_dot, v2_dot predictions from the SINDy model with the actual X_dot.
        """
        sigma1, sigma2 = theta

        # Reshape U to (N,1) if it's 1D
        if U is not None and U.ndim == 1:
            U_reshaped = U[:, None]
        else:
            U_reshaped = U

        # SINDy "predict" => x_dot if fit with x_dot=f(X,U).
        X_dot_pred = self.model.predict(X, u=U_reshaped)
