...
# MPC Parameters
N_values = [20, 10, 5]    # Prediction horizons to study
N_labels = ['N = 20', 'N = 10', 'N = 5']
...
def mpc_cost(u_sequence, current_state, reference, Q, R):
    cost = 0
...
# Initialize dictionaries to store simulation results for each N
results_N = {}
...
for label in N_labels:
    plt.plot(t_sim[:len(results_N[label]['X_sim'])], results_N[label]['X_sim'][:, 0], label=label)
plt.plot(t_sim[:len(results_N[label]['X_sim'])], np.zeros_like(t_sim[:len(results_N[label]['X_sim'])]), 'k--', label='Reference')
plt.ylabel('Position (m)')
plt.title('MPC: Position vs Time for Different Prediction Horizons')
plt.legend()
plt.grid(True)
...
plt.subplot(4,1,2)
for label in N_labels:
    plt.plot(t_sim[:len(results_N[label]['X_sim'])], results_N[label]['X_sim'][:, 1], label=label)
plt.plot(t_sim[:len(results_N[label]['X_sim'])], np.zeros_like(t_sim[:len(results_N[label]['X_sim'])]), 'k--', label='Reference')
plt.ylabel('Velocity (m/s)')
plt.title('MPC: Velocity vs Time for Different Prediction Horizons')
plt.legend()
plt.grid(True)
...
plt.subplot(4,1,3)
for label in N_labels:
    plt.plot(t_sim[:len(results_N[label]['U_sim'])], results_N[label]['U_sim'], label=label)
plt.axhline(y=u_max, color='r', linestyle='--', alpha=0.3, label='Control Limits')
plt.axhline(y=u_min, color='r', linestyle='--', alpha=0.3)
plt.ylabel('Control Input (N)')
plt.title('MPC: Control Input vs Time for Different Prediction Horizons')
plt.legend()
plt.grid(True)
...
for label in N_labels:
    plt.plot(results_N[label]['X_sim'][:, 0], results_N[label]['X_sim'][:, 1], label=label)
plt.plot(x_ref[0], x_ref[1], 'k*', markersize=10, label='Reference')
plt.xlabel('Position (m)')
plt.ylabel('Velocity (m/s)')
plt.title('MPC: Phase Space Trajectories for Different Prediction Horizons')
plt.legend()
plt.grid(True)
plt.show()
