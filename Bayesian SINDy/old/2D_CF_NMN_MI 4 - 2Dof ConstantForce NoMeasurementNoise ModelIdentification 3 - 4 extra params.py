...
class BayesianSINDy:
    def __init__(self, n_params=19, n_walkers=64, b=0.01):
    ...
    def construct_library(self, X):
        ...
        Includes a constant term, linear terms, and nonlinear terms (x^2, v^2).
        """
        x1 = X[:, 0]
        v1 = X[:, 1]
        x2 = X[:, 2]
        ...
        # Candidate functions: [constant, x1, v1, x2, v2, x1^2, x2^2, v1^2, v2^2]
        Theta = np.column_stack([
            ...
            v1**2,             # Useless nonlinear term
            v2**2              # Useless nonlinear term
        ])

        self.feature_names = ['1', 'x1', 'v1', 'x2', 'v2', 'x1^2', 'x2^2', 'v1^2', 'v2^2']
    ...
    def log_prior(self, theta):
        ...
        """
        # Unpack parameters
        coeffs_f1 = theta[:9]    # Coefficients for dv1/dt
        coeffs_f2 = theta[9:18]  # Coefficients for dv2/dt
        sigma_epsilon = theta[18]  # Process noise
    ...
    def log_likelihood(self, theta, Theta, X_dot):
        ...
        try:
            ...
            coeffs_f1 = theta[:9]    # Coefficients for dv1/dt
            coeffs_f2 = theta[9:18]  # Coefficients for dv2/dt
            sigma_epsilon = theta[18]  # Process noise standard deviation
    ...
    def fit(self, X, X_dot, n_steps=5000):
        ...
        # Number of parameters: coefficients_f1 (9) + coefficients_f2 (9) + sigma_epsilon (1) =19
        self.n_params = 19

        # Initial guess for parameters
        # Start with zeros for coefficients and 1.0 for sigma_epsilon
        initial_guess = np.concatenate([
            np.zeros(9),  # coeffs_f1
            np.zeros(9),  # coeffs_f2
    ...
    def predict(self, x0, t, n_samples=100):
        ...
        for theta in tqdm(theta_samples, desc="Generating predictions"):
            # Unpack parameters
            coeffs_f1 = theta[:9]
            coeffs_f2 = theta[9:18]
            sigma_epsilon = theta[18]
            ...
            for i in range(len(t) - 1):
                ...
                Theta_current = np.array([1, x1, v1, x2, v2, x1**2, x2**2, v1**2, v2**2])
...
def compute_true_coefficients(m1, m2, k1, k2, c1, c2, constant_force):
    ...
    f1_v1_sq = 0         # True coefficient for v1^2 term is zero
    f1_v2_sq = 0         # True coefficient for v2^2 term is zero
    ...
    f2_v1_sq = 0                # True coefficient for v1^2 term is zero
    f2_v2_sq = 0                # True coefficient for v2^2 term is zero

    # Combine into coefficient vectors
    true_coeffs_f1 = [f1_1, f1_x1, f1_v1, f1_x2, f1_v2, f1_x1_sq, f1_x2_sq, f1_v1_sq, f1_v2_sq]
    true_coeffs_f2 = [f2_1, f2_x1, f2_v1, f2_x2, f2_v2, f2_x1_sq, f2_x2_sq, f2_v1_sq, f2_v2_sq]
...
if __name__ == "__main__":
    ...
    model = BayesianSINDy(n_params=19, n_walkers=128, b=0.005)
